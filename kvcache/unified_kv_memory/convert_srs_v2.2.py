...
def main():
    ...
    # Merge new requirements into the base tables
    for table in tables:
        section = table['section']
        if section in additions:
            print(f"\nMerging additions for sheet: {section}...")
            total_added = 0
    ...
    dict_updates = {
        ('L2', '统一协议与标准接口'): '统一 KVConnector Protocol 与标准数据结构，支持三阶段发布与语义兼容性校验。',
        ...
        ('L3', '前缀目录与元数据平面'): '分布式前缀索引与元数据平面，支持副本位置状态、可消费判定、墓碑失效、紧凑布局与分片策略。',
        ('L3', '语义策略与路径管理'): '统一传输语义与策略引擎，支持最优位置解析、查询快路径与传输描述符自动生成。',
        ('L3', '一致性、隔离与QoS'): '一致发布、多租户隔离与 QoS，支持读写租约、可见性控制、发布提交日志、副本隔离与 GC 回收。',
        ...
        ('L4', 'Fabric能力与统一路由'): '硬件能力表、地址转换与混合 Fabric 路由，支持底层布局能力描述与约束声明。',
    ...
    for table in tables:
        ...
        for r_idx, row_data in enumerate(table['rows'], 2):
            ...
            if section == '横向模块分类':
                ...
                if tm_match:
                    ...
                    # Col 3 (L1), Col 4 (L2), Col 5 (L3), Col 6 (L4) in 1-based indexing
                    # Col 3: index 2, Col 4: index 3, Col 5: index 4, Col 6: index 5
        ...
        for c_idx in range(1, len(table['header']) + 1):
            ...
            if any(kw in header_col for kw in ['ID', '标识', '需求唯一标识']):
                ...
                width = 20
            elif any(kw in header_col for kw in ['描述', '要点', '职责', '模块核心职责描述 (第一性原理定位)', '模块核心职责描述 (第一性原理定位)']):
                # Double width constraint for long description
                width = 55
            elif any(kw in header_col for kw in ['场景', '痛点', '指标', '验收方法', 'L推理调度层需求 ID', 'LKVConnector 层需求 ID', 'L传输管理层需求 ID', 'L底层传输层需求 ID']):
                # Increased width for the mapping columns
                width = 40
            else:
