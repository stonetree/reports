...
> 版本：第一版（依据《KVCache统一异构软件设计说明书C_专业性与逻辑审阅意见_reviewed.md》修订）  
> 评审基线：《KVCache SRS需求列表 V2.1.xlsx》  
> 文档用途：帮助第三方评审专家理解项目愿景、需求设置初衷、总体架构、阶段路径、验收逻辑与评审重点。

## 1. 文档定位与使用方法

本文档是《KVCache SRS需求列表 V2.1.xlsx》的总体架构导读，不替代 SRS 本身，也不替代后续详细设计、接口 IDL、测试方案、部署手册或硬件选型规格。

本文档的主要编制依据包括：《KVCache SRS需求列表 V2.1.xlsx》《KVCache统一异构软件设计说明书 A.md》《KVCache统一异构软件设计说明书 B.md》《KVCache统一异构软件设计说明书 C.md》《KVCache统一异构软件设计说明书C_专业性与逻辑审阅意见_reviewed.md》《KVCache_Architecture_SRS_Report.docx》《KVCache_Strategic_Value_Report.docx》及 `unified_kvcache_汇报材料_v2.md`。其中，需求数量、阶段、指标和工程基线以 SRS V2.1 为事实基线；愿景性材料仅用于解释背景，不覆盖 SRS 的正式边界。
...
以上分级不改变 SRS 当前优先级，只表示这些信息对架构定版、主流程接入和生产验收的重要程度。
...
KVAccessIntent
  ...
  -> QueryPlan
  -> Lease/AttachHandle + ExtentManifest/Descriptor
  -> Completion/Fence
  -> Framework Consume
  -> Metrics/Trace/Cost Model Calibration
```
