...
本次刷新以 V2.3.1 全量需求树为分解基线，以第一方工程落地与业务价值为核心原则。它不是一次编号替换，而是把“为什么立项、为什么由第一方承担、SR 为什么这样聚合、原型如何证明系统落地价值”连接成同一条可审计链。评审首先应确认以下六点。
...
6. **立项前 8 个 PVT 与 3 个 CVT 是四个核心验证阶段 (E0–E3) 的量化验证目标，不是既有成绩。** 所有比较必须采用同硬件、同模型、同 SLO、同调优预算的公平基线，并包含最佳可行开源组合、关闭状态智能和移除微架构信息等关键消融。

因此，本轮重点在于“分阶段开展工程验证，并在 4～5 周内获取扎实的第一手实测工程数据、摸清技术边界”。未达到 E3 前不得宣称已经优于开源，未达到 E4 前不得承诺客户 ROI，未达到 E5 前不得宣称形成持续技术优势。
...
| 4 | V3.2 工程落地与技术方案分析、立项汇报材料 | 冻结第一方责任、客户价值北极星、公平竞争基线和潜在壁垒的最高叙事原则 |
