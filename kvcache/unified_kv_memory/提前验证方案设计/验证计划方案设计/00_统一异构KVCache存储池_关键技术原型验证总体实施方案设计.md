...
│  🔴  │ PVT-04 │ 微秒级动态决策引擎与 CostEvaluator 成本模型     │ 6 人日   │ 覆盖 DeepSeek MLA (~35KB/tok) 与 Qwen MHA (320KB/tok) │
...
├──────────────────────┼────────────────────────┼────────────────────────┼────────────────────────────────────────────┤
│ **MLA 潜在注意力**   │ DeepSeek-V3 / R1 (FP8) │ **~35 KB / token**     │ 通信数据量显著收缩，重点考核微秒调度决策   │
...
│   ├── make_workload.py           # 构造 MLA/MHA 受控数据集 (支持 DeepSeek MLA 与 Qwen MHA)
