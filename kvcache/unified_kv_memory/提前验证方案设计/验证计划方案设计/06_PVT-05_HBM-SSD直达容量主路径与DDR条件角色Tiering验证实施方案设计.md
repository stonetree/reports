...
> **公共执行契约**：本项遵循 [Benchmark 公共契约与证据分级规范](./Benchmark公共契约与证据分级规范.md)。每个测试 `run_id` 需在运行前严格固化物理设备、文件系统或裸盘路径、数据块尺寸 (Block Size)、数据传输路径、硬件队列深度、并发请求数、显存超配比例 (Overcommit Ratio)、预热轮次、样本总量及目标证据等级；实测产出必须完整保留底层原始 I/O 事件、硬件实际完成字节数、异常失败归因、实测物理路径 (`actual_path`)、Host CPU/DDR 动态观测数据及规范的状态判定枚举。凡出现关键字段缺失、依赖脚本静态预置数值、无法与底层设备性能计数器对账，或 `actual_path` 与 `planned_path` 发生非预期偏离等情况，一律不纳入 `MEASURED` 生产级汇总。
...
> - **Host Payload Touch Bytes**：Host CPU 或 Host DDR 触碰正文数据的字节数，目标直达路径严格要求为 0，且需由动态探针或硬件性能计数器严密证明；
...
> **开源基线版本**：Mooncake `f90ae691f109e49a60920e0c8abbf7e572826d8c`。正式测试结果需绑定实际 I/O 后端引擎、底层驱动/运行时版本、NVMe 设备型号、加速器芯片及配置哈希。
...
在传统大模型推理系统的显存分层换出方案中，冷 KVCache 数据通常先通过 PCIe DMA 拷贝至主机内存 (Host DDR)，再由 Host CPU 或操作系统内核块设备层二次中转转发至 NVMe SSD；当缓存命中发生换入时，数据则沿着相反方向再次经历两次内存搬运：
...
该传统路径不仅消耗 CPU 算力，且频繁搬运数据会剧烈争用 Host DDR 内存带宽与 L3 缓存，干扰在线推理的控制面调度与分布式通信。

因此，本方案设计了控制面与数据面双流分离的架构：
...
系统中的“直达 (Bypass DDR)”属性由底层真实的 DMA 目标物理显存地址、硬件完成中断记录及 Host CPU 零数据拷贝探针共同证明。
...
| `原型验证代码/PVT-05/Makefile` | 仅提供当前 C++ 测试桩的编译规则，使用标准 g++ 编译 | 未链接 `liburing`、SPDK、GDS 或加速器驱动运行时库 |
| PVT-05 源码目录 | 当前尚未包含 `tier_allocator.h` 或 `eval_tiering.py` | 早期规划的分层分配器头文件与评估脚本在当前仓库中尚未受控提供 |

说明：`tier_storage_bench.cc` 当前属于演示数据结构与输出 Schema 的 DEMO 工具，后续需逐步替换为对接真实 SPDK/io_uring 存储通路与 Host Touch 动态探针。

### 0.3 当前 DEMO 模式的输出语义与证据边界

当前受控 C++ 基准程序在运行后固定生成 `nvme_direct_bypass_ddr` 与 `hbm_ddr_ssd_staging` 两个演示数据行。其中，direct 模式下输出的 190.4 Gbps 吞吐及 1.2% Host CPU 占用率仅为预设演示数值；staging 模式下输出的 85.2 Gbps 吞吐与 65.4% CPU 占用率同样为模拟参数。配套的 Python 评估脚本在各模式下同样写入了固定的请求完成数、OOM 发生率及 DDR 内存占用数值。

上述演示数据仅用于验证输出 CSV 字段格式的完整性与报表分类逻辑，不作为宣称显存容量扩展比例或 SSD 实际读写带宽的技术依据。
...
|---|---|---|
| 直达数据面验证 | NVMe SSD 与 NPU HBM 之间的数据流转是否真正绕过 Host DDR | 真实块设备 I/O、真实显存物理地址、DMA 硬件完成中断、Host Payload Touch 动态探针 |
| 分层控制面流转 | 热块异步换出、冷块按需换入、高低水位线触发及租约生命周期是否清晰可溯 | 连续块区间描述符、分层状态迁移全量日志、硬件队列深度、容量回收触发归因 |
| 显存容量扩展收益 | 在显存超配 (Overcommit) 逐步提升时，HBM 可服务上下文容量、OOM 发生率及请求抢占率变化情况 | 相同请求流量、相同模型权重、相同 HBM 物理配额下的公平 A/B 原始采样记录 |
...
每个正式 `run_id` 交付：
1. **《存储与计算硬件拓扑配置快照》**：记录 PCIe 拓扑关系、NUMA 绑定、驱动版本、文件系统挂载参数或裸盘声明；
...
工程实现约束：
- `kv_virtual_addr` 为设备控制器与 DMA 引擎均可直接物理访问的 IOVA 或显存句柄；
- `nvme_lba` 严格按照磁盘设备的物理扇区边界进行对齐；
- `generation` 与 `lease_expire_ns` 形成双重保护，防止陈旧失效的后台 I/O 完成事件错误覆盖新分配的显存块；
- 发生 `INVALID` 状态、I/O 超时或硬件故障时，安全触发本地算子重算或向 Host DDR 条件回退；
- 描述符自身在 Host 内存中占用的控制元数据开销，不计入正文数据的 Host Payload Touch。

### 2.2 分层块分配器状态迁移设计

统一管理 HBM、NVMe SSD 及条件回退 DDR 的分层块分配器（TierBlockAllocator）遵循以下状态迁移图谱：
...
设单块大小为 $B$、待存储的 KV 数据总量为 $S$，分配块数与对齐字节数计算如下：

$$
\text{blocks} = \left\lceil \frac{S}{B} \right\rceil, \quad \text{aligned\_bytes} = \text{blocks} \times B
$$

当且仅当 $B$ 为 4KB 的整数倍，且 `nvme_lba * sector_bytes`、文件偏移 `offset` 及传输长度 `length` 均满足底层块设备的物理对齐约束时，系统方可进入真实 Direct I/O 阶段。

### 2.4 显存高低水位线策略

显存池推荐配置高水位线 $H_{\text{high}}$（如已用显存 85%）与低水位线 $H_{\text{low}}$（如已用显存 70%）：
- 当 `used_hbm >= H_high`：挑选 LRU 冷数据块，启动后台异步批量换出，直至 `used_hbm <= H_low`；
- 当 `used_hbm <= H_low`：停止后台换出，将 PCIe 与存储 I/O 带宽保留给前台在线推理。
...
|---|---|---|---|
| `pure_hbm` | 所有 KVCache 严格保留在本地 HBM 显存池中，作为容量与时延的基线 | Python 脚本仅输出固定演示统计 | 真实显存分配与相同的在线请求流量 |
...
|---|---|---|
| HBM 物理配额 | 设备物理显存可用容量的 50%、70%、90% | 模拟不同显存水位下的容量压力边界 |
| 超配比例 (Overcommit) | 1.0×、1.25×、1.5×、2.0× | 检验显存池超出物理容量时的分层支撑能力 |
| 并发请求数 | 1、4、16、32 并发 | 观测不同并发负载下的 I/O 队列与显存争用 |
| KV 块尺寸 (Block Size) | 1MB、4MB、16MB、64MB | 保持 4KB 对齐，分析不同块大小下的总线吞吐与碎片率 |
| I/O 硬件队列深度 (QD) | 1、4、16、32 深度 | 观测底层存储控制器的并发吞吐与排队时延 |
| 测试请求样本量 | 每测试点至少完成 500 个有效请求 | 预热阶段请求与稳态评测请求严格隔离记录 |
| 访问局部性分布 | 热点复用率 70%、90%；冷块换入比例 10%、30% | 构造真实大模型多轮对话与知识库检索的访存分布 |
| 后台存储压力 | 无后台 I/O、半饱和带宽、满载可用带宽 | 检验前后台混压下的抗干扰能力 |

### 3.3 物理运行环境严格冻结

在每个测试点执行前，自动采集并持久化以下环境快照：
- 导出 NPU 加速卡、NVMe 控制器、PCIe Root Complex 拓扑及 NUMA 亲和性关系；
- NVMe 固态硬盘型号、固件版本、物理扇区大小、文件系统挂载参数或裸盘标识；
- 加速器驱动版本、运行时环境、GDS 模块或内核驱动加载状态；
- CPU 核心亲和性绑定、操作系统 I/O 调度器及透明大页 (THP) 配置；
- 前台在线推理服务引擎版本与网络通信配置；
...
当前受控工程 `Makefile` 仅编译 `tier_storage_bench.cc` 的单机 DEMO 程序，未链接 `liburing`、SPDK、GDS 或加速器驱动运行时。

面向生产级评测的最小工程增量：
1. **底层存储 I/O 引擎选型固化**：固化基于 `io_uring` 固定缓冲区或 SPDK NVMe 驱动的用户态直达实现；
2. **真实 NVMe 读写与中断采集**：实现向真实磁盘提交 Direct I/O 操作，采集提交时间戳、完成中断、实际字节数及硬件错误码；
3. **真实显存缓冲区映射**：将 I/O 目标地址绑定至 NPU 真实分配的 HBM 显存物理地址；
4. **Host CPU 零拷贝探针接入**：挂载 eBPF 动态探针，精准采集 Host Payload Touch 字节数；
5. **真实推理打流与时间线对齐**：对接在线推理流量，将后台分层换入换出事件与前台推理请求的 TTFT/TPOT 时间线实现关联；
...
- **操作意图**：确保实测数据可复现并追溯至唯一的源码版本、硬件拓扑及配置参数。
- **执行命令**：
...
- **应观察现象**：输出 CSV 包含 direct 与 staging 两类演示行，状态标记为 `DEMO,DEMO_ONLY`。
...
- **应观察现象**：导出的 JSON 文件中 `evidence_level` 标注为 `DEMO`，`status` 标注为 `DEMO_ONLY`。

### 步骤 3：核对并显式登记证据缺口

- **操作意图**：在投入真实硬件测试前，梳理当前原型代码与生产级要求之间的技术差距，明确当前未支持特性。
- **执行命令**：
...
- **前置条件**：真实 NVMe 块设备已挂载；`io_uring` 或 SPDK 驱动环境就绪。
- **操作意图**：在单数据块级别证实 NVMe 控制器到 NPU 显存物理地址的直达数据通路，验证 4KB 对齐与数据完整性。
- **执行动作**：针对 4KB、1MB、4MB 三种对齐长度分别执行单读、单写及读后数据校验；记录 `submit_ts`、`complete_ts`、`nvme_lba`、目标显存物理地址及实际完成字节数；同步采集 Host Touch 探针数据。

### 步骤 5：运行四模式同场景 A/B 压力实测（条件步骤）

- **操作意图**：在真实推理流量下对比四种模式的显存利用率、换入换出吞吐、前台时延及 Host DDR 的角色分工。
- **执行动作**：在固定模型权重、Prompt 随机种子、KV 块尺寸、HBM 物理配额及请求序列下，依次运行 `pure_hbm` 基线与三种分层模式；全流程采集前台 TTFT、TPOT、SLO 达标率、OOM 次数、后台 I/O 吞吐及 Host Touch 计数。

### 步骤 6：全链路前后台混压与异常回退（条件步骤）

- **操作意图**：确证后台分层 I/O 在满载时不会无条件破坏前台在线推理的 TPOT 稳定性，并验证 Host DDR 仅在异常回退时被条件性启用。
- **执行动作**：以前台无后台 I/O 作为基准；增加后台换出与换入压力直至设备饱和；主动注入 I/O 超时、租约失效及短 I/O 异常；检验系统是否平稳触发安全回退。

### 步骤 7：标准证据包归档与复核对账

- **操作意图**：将全量原始数据、硬件拓扑快照、状态迁移日志及异常记录标准化归档。
- **执行动作**：在 `results/pvt05/<run_id>/` 目录下归档 `tier_storage.csv`、`mode_results.jsonl`、`block_state_transitions.jsonl`、`io_completion.jsonl`、`host_payload_touch.jsonl`、`foreground_latency.csv`、`errors.log` 及 `summary.md`。
...
|---|---:|---|
| 可服务上下文容量 | 同等 HBM 配额下，分层系统可服务 KV 容量相对纯 HBM 基线提升 $\ge 30\%$ | 基于真实完成的推理请求与相同请求序列计算 |
| OOM 与请求抢占抑制 | 在 1.5× 显存超配压力下，OOM 次数与抢占率相对基线下降 $\ge 50\%$ | 保持相同请求量与负载分布 |
| NVMe 直达有效带宽 | 稳态 Direct 路径顺序读写带宽达到物理设备标称峰值的 $\ge 80\%$ | 明确标注读写比例、块尺寸及硬件队列深度 |
| Host CPU 正文拷贝 | 正常 `hbm_ssd_direct` 直达路径下 Host Payload Touch 严格 $= 0$ | 由动态探针或硬件计数器完整覆盖数据传输周期 |
| 前台 TPOT 干扰率 | 满载混压下前台在线推理 TPOT P99 恶化率 $< 3\%$ | 在统一的硬件 QoS 与统计窗口下评估 |
| 语义与代际一致性 | 所有就绪块 100% 通过语义与代际校验，陈旧失效数据 0 错误消费 | 发生陈旧脏数据读取即直接判定为不通过 |

### 7.2 状态判定枚举与规则

- **GO（全链路证据闭环）**：关键硬件路径打通，候选准入门槛达标，数据一致性与零拷贝校验 100% 通过；
- **CONDITIONAL（局部硬件或场景达标）**：局部路径成立，但特定拓扑、QoS 流控或某项次要门限受限；
- **NO-GO（关键路径未达标或发生数据损坏）**：直达路径未打通、发生陈旧数据错误消费，或前台 TPOT 干扰超出止损门限；
- **NOT-SUPPORTED（物理环境未支持）**：现场硬件、底层驱动或运行时环境缺乏目标直达能力；
- **INVALID-EVIDENCE（无效证据）**：原始证据缺失、关键字段为空、静态预置数值冒充实测、计划路径与实际路径不一致。

### 7.3 立即安全止损条件

在测试过程中凡触发以下任一异常，立即终止测试并保存现场：
- NVMe 磁盘返回短 I/O 错误、介质坏块错误或硬件完成数与提交数不一致；
- NPU 显存物理目标地址校验失败，或发生 DMA 非法越界写入；
- `hbm_ssd_direct` 直达路径测得的 Host Payload Touch 大于 0 且缺乏合理的物理归因；
- 已过期的陈旧 Generation 代际块或未完成写入的半写块被错误标记为 `READY` 并被下游算子消费；
- 前台在线推理 TPOT 尾部时延持续飙升超出预设止损红线；
- 存储设备温度或 SMART 健康计数器超出安全运行范围。
...
|---|---|---|---|
| E0（接口与证据规范确认） | 核对四模式 Schema、CLI 参数、结果目录结构及无效证据拦截逻辑 | 源码审计报告、DEMO 输出 CSV/JSON、规范化 manifest | 确认当前代码边界，杜绝将测试桩冒充实测 |
...
| 条件证伪（DDR 角色与依赖） | 针对 GDS 直达、io_uring/SPDK、NUMA 及 PCIe 拓扑开展严格证伪，摸清技术边界 | 软硬件依赖对照矩阵、降级回退边界报告 | 若硬件不支持直达则降级为 DDR 中转 |
...
- **AI Agent 协同职责**：
  ...
  4. 严守技术诚信红线，严禁虚构驱动接口、伪造磁盘吞吐或编造零拷贝证据。
...
- **direct 与 staging 模式输出数值固定且无设备错误**：受控源码处于 DEMO 占位阶段的预期表现；测试结果应规范标记为 `DEMO`。
- **导出的 `host_payload_touch_bytes` 字段为空**：空值代表尚未接入完整的动态探针，绝不代表零数据拷贝成立；该测试点必须记录为 `INVALID-EVIDENCE`。
- **执行命令传入 `--evidence-level` 参数报错**：受控 Python 脚本 CLI 尚未实现该参数；应按照实际支持的 CLI 参数执行。
- **工程中找不到 `tier_allocator.h` 头文件**：当前仓库中尚未包含分层分配器实现；在未完成代码开发前，不宣称已完成 Watermark 水位线自动换出验证。
- **提升硬件队列深度 QD 但设备吞吐与完成数无变化**：排查 `qd` 是否仅为打印字段，确认底层后端驱动是否启用了异步并发硬件队列。
- **测试模式标记为 direct 但探针测得非零正文内存触碰**：说明底层驱动发生了静默回退，实际执行了 Host DDR 中转；必须将 `actual_path` 修正为中转模式并判定为 `NO-GO` 或 `INVALID-EVIDENCE`。
- **测试中 OOM 发生率大幅下降但实际完成请求数显著减少**：说明 A/B 对照的工作负载不一致；必须废弃该测试点，重新冻结请求流量后重新测试。
- **注入异常后陈旧代际 (Old Generation) 数据块依然被算子消费**：说明代际校验与租约安全守卫存在缺陷；必须立即终止测试，判定为一票否决的 `NO-GO` 并紧急修复状态机。
