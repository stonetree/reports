...
> **开源基线版本**：Mooncake `f90ae691f109e49a60920e0c8abbf7e572826d8c`；vLLM `842dd8fd96650063e1ad32e6075742d457d39773`。正式测试结果必须绑定实际被测系统、通信库版本、底层驱动及配置哈希。
...
在大模型分布式推理架构中，KVCache 绝非仅仅由 Prompt 原始文本决定的通用字节数组。模型结构微调、Tokenizer 词表变更、Prompt 模板微小差异、LoRA 适配器切换以及显存块的写入生命周期，都会直接决定一段缓存张量在数学语义上是否允许被当前推理上下文安全消费。若仅在应用层比对缓存键哈希或对象 ID，极易将表面相似但实际数学权重完全不兼容的脏数据接入计算流，导致模型输出逻辑错乱或产生毁灭性的幻觉。

在张量并行 (TP=8) 多卡协同场景下，控制面更面临严苛的分布式同步约束：8 张物理加速卡必须在进入下一次集合通信 (AllReduce/AllGather) 前，对“是复用加载缓存数据”还是“全体执行本地直接重算”达成绝对一致的决策。一旦 Rank 0~6 判定命中并进入加载分支，而 Rank 7 因局部校验失败擅自进入重算分支，多卡之间在集合通信算子上的调用时序与张量尺寸将发生严重错位，瞬间引发整个推理集群的永久性死锁挂起。因此，多卡状态同步机制必须与 6 维语义校验深度咬合，确保“同生共死，全卡步调绝对一致”。

### 0.2 六维语义与全生命周期检查体系

生产级 ConsumeEligibility 校验引擎对命中的 KVCache 严格执行以下 6 维微秒级漏斗式合法性检查：

```text
第 1 维：模型版本 (Model Version) ──────────────► 验证基础模型架构与权重版本是否完全匹配
第 2 维：Tokenizer 词表哈希 (Tokenizer Hash) ────► 验证分词算法与编码词表指纹是否绝对一致
第 3 维：Prompt 模板哈希 (Template Hash) ───────► 验证 System/User/Assistant 模板组织结构是否匹配
第 4 维：LoRA 适配器标识 (Adapter ID) ──────────► 验证微调权重与插槽标识是否完全吻合
第 5 维：Ready 写入就绪位 (Ready Bit) ──────────► 验证后台 DMA 搬运已彻底完成且内存屏障已生效
第 6 维：ViewLease 租约有效性 (Lease Validity) ──► 验证远端显存对象租约未过期且未被主动撤销
```

每一个维度的校验失败均必须对应独立的错误枚举码，以便精准审计、故障归因及触发针对性的回退策略。仅有 6 维全部通过方可放行消费；任何一维异常均坚决阻断。
...
单字节位图必须绑定严格的 `consensus_epoch`（共识轮次）、`protocol_version`（协议版本）及各 Rank 的心跳时间戳，严禁因上一轮残留脏位或异常退出进程引发误判。

### 0.4 当前受控源码能力矩阵审计

| 源码文件与路径 | 当前受控源码实际行为 | 现阶段尚不能声称的能力 |
|---|---|---|
| `原型验证代码/PVT-06/consume_eligibility.h` | `SemanticMetadata` 仅包含 4 个字符串、租约时间戳及 `bool ready_bit`；`CheckResult` 定义了 `PASS` 与 6 个拒绝枚举 | 未包含 Token ID 序列对比、Generation 代际版本号、部分挂载执行计划、原子内存序及 xxHash64 哈希计算 |
...
`rank_consensus_bench.cc` 输出的 `P50/P99` 仅是对单进程内部局部代码段计时后机械累加静态常数所得的演示数据，绝非 8 个独立物理进程在真实总线与网络上的同步分位数。其操作的 `rank_bitmap` 属于单进程内部的栈内存，无法代表跨卡进程间的真实共享。此外，命令行中传入的 `--ranks`、`--loops`、`--inject-divergence` 及 `--out` 等参数在当前代码中并未实际解析。

`test_correctness.py` 在未显式传入 `--sut-command` 时，仅将测试用例静态写入结果文件，此时 `actual` 字段为 `null`，`executed_cases` 为 0，且脚本以状态码 2 退出。因此，该脚本属于标准化测试 Harness 脚手架，不可直接将其静态输出作为证明系统已消除错误消费的生产级依据。
...
|---|---|---|
| 六维语义校验正确性 | 8 类语义冲突、生命周期过期及状态未就绪用例是否均被 100% 精准拦截或安全回退 | 每一个测试用例实际调用 SUT 的输入输出日志、独立 Oracle 判决结果、错误消费绝对计数 |
| 正常语义消费放行 | 在 6 维语义完全匹配且数据就绪的前提下，系统是否平稳放行缓存消费 | 正向用例执行日志、模型/租约/Ready 快照凭证、数据一致性校验记录 |
| 部分前缀安全挂载 | 当长 Prompt 前缀完全匹配而尾部发生语义冲突时，系统能否安全挂载匹配前缀并增量重算尾部 | Partial Attach 拆分执行计划、匹配前缀长度、尾部重算任务日志及最终张量输出校验 |
| 多卡状态同步一致性 | 在张量并行 TP=8 环境下，8 个独立 Rank 是否在同一 epoch 内达成步调一致的决策 | 8 个 Rank 独立记录的决策日志、Epoch 轮次标识、超时拦截日志、进程退出状态码 |
| 集合通信死锁防护 | 在单卡慢、卡分歧或特定 Rank 异常崩溃注水场景下，后续集合通信算子是否能平稳退出或安全完成 | 集合通信调用开始/完成时间戳、底层错误码、系统死锁监控日志、看门狗超时记录 |
| 微秒级极速控制面 | 6 维语义校验与多卡状态同步耗时是否稳定达到微秒级工程准入门限 | 逐样本原始计时日志、CPU 核心亲和性绑定记录、P50/P95/P99 权威统计分布 |
...
在采用哈希指纹时，必须严格固化 xxHash64 的种子 (Seed)、输入字节流标准化规范及哈希冲突兜底机制；哈希字段不可掩盖词表与模板的物理来源。

### 2.2 标准返回结果枚举定义

校验引擎与共识模块对外输出的标准状态枚举定义如下：
...
当前受控 C++ 代码仅实现了前 7 个枚举值。对于 C07（部分挂载）与 C08（协同回退），在正式测试前必须先补齐底层接口支持；在未完成扩展前运行此类用例，必须规范标记为 `NOT-SUPPORTED` 或 `INVALID-EVIDENCE`。

### 2.3 六维校验物理边界与仲裁准则

在执行 6 维语义校验时，必须严格遵循以下物理仲裁边界：

- 模型、Tokenizer、模板及适配器的比对必须严格遵循规范化字节流的一致性；
- `ready_bit` 未就绪（`false`）时，必须优先于租约过期抛出 `REJECT_NOT_READY`，确保半写脏块在第一时间被拦截；
- 时间戳校验采用 `now_ms >= lease_expire_ms`，达到或超过到期时间戳的租约一律视为已失效；
- `ready_bit` 的读写必须具备原子内存序（Release 写入、Acquire 读取），确保在多核与异构设备间的内存可见性；
- 发生代际 `generation` 不匹配时，一票否决拒绝消费并主动回退至本地重算。

### 2.4 张量并行 Rank 状态机设计

张量并行 TP=8 各 Rank 必须严格遵循以下状态机流转：
...
在进入 `COLLECTIVE_ENTER` 之前，全量 Rank 必须在物理上达成同一 `consensus_epoch` 的全局决断；发生任何分歧、超时或异常，默认动作一律为全体平稳回退至本地重算。
...
每个冲突用例均必须配备一条正向对齐样本，证明仅在目标字段发生异动时系统能准确产生状态翻转。当多个维度同时发生冲突时，必须严格按照 6 维漏斗的预设优先级输出首要拒绝原因。

### 3.2 TP=8 状态同步场景矩阵

| 评测场景分类 | 各 Rank 状态分布特征 | 预期全卡统一步调 | 必须采集的底层物理证据 |
|---|---|---|---|
| 全量命中场景 | 8/8 Rank 全部通过校验 | 8 卡同步执行缓存加载 | 8 卡同一 Epoch 记录、统一下发加载指令、集合通信平稳完成 |
| 单卡分歧场景 | 7 卡通过 / 1 卡发生语义拒绝 | 8 卡步调一致回退重算 | 状态分歧被瞬间捕获、全量放弃加载、无任何单卡擅自进入孤立路径 |
| 慢卡超时场景 | 7 卡准时 / 1 卡执行严重超时 | 8 卡在超时截断后统一重算 | 看门狗超时中断、超时原因归因、各卡在超时后平稳收敛 |
| 进程崩溃场景 | 7 卡存活 / 1 卡发生突发 SIGKILL | 7 卡安全捕获并报错退出 | 进程心跳监测日志、死锁安全破除、共享内存资源被安全释放 |
| Epoch 错位场景 | 新旧不同轮次的位图混用 | 坚决拒绝旧轮次并重新同步 | Epoch 冲突拦截日志、代际隔离验证、共享内存原子重置 |
| 协议版本冲突 | 节点间协议版本 A/B 冲突 | 阻断消费并显式报错 | 协议版本握手日志、安全拒绝凭证、杜绝错误解析 |

### 3.3 物理测试参数严格冻结

| 测试维度 | 正式实施计划标准 | 工程说明与约束 |
|---|---:|---|
| 张量并行规模 (`world_size`) | 严格 $= 8$ | 真实的 TP=8 结论必须基于 8 个独立物理 Rank 采集；缩小规模仅可作为 LAB 结论 |
| 正确性测试轮次 | 每个用例至少重复 100 轮 | 严格检验边界拦截的绝对稳定性与进程复用安全性 |
| 状态同步测试轮次 | 10000 轮起步，正式目标 100000 轮 | 基于海量重复采样获取高精度的微秒级 P99 尾部时延分布 |
| 同步超时时限 (Timeout) | 基准设定 $500\mu	ext{s}$，依设备微调 | 超时门限必须写入测试元数据清单，严禁在代码中隐式写死 |
| CPU 核心亲和性绑定 | 严格将 8 个 Rank 绑定至独立 CPU/NUMA 核心 | 消除操作系统调度争用对微秒级同步时延的非预期干扰 |
| 候选准入门槛 | 校验 P99 $< 5\mu	ext{s}$；同步 P99 $< 100\mu	ext{s}$ | 候选工程门限，非当前测试桩已达到的预置结果 |
...
特别说明：执行 `make clean` 会自动清理当前目录下的可执行程序与 CSV 文件；测试前必须确保历史数据已安全归档至独立结果目录。当前 `Makefile` 尚未链接 POSIX 共享内存库或分布式通信库。
...
- 输出结果硬编码写入当前目录的 `res_consensus_summary.csv`；
- 输出 CSV 状态固定标记为 `DEMO,DEMO_ONLY`。

因此，向命令行传入 `--ranks 8` 或 `--inject-divergence rank7` 仅可作为参数解析审计样例，不可作为多卡实测依据。真实实验必须补齐多进程编排、共享内存与故障注水实现。

### 4.3 当前测试脚手架行为审计

当前受控 `test_correctness.py` 仅支持 `--sut-command <command>` 与 `--out <path>` 两个参数。在未提供被测命令时，脚本不会实际调用被测程序。当传入被测命令后，脚本通过标准输入输出交互比对返回状态。当前工程中尚未受控提供独立的 `eval_consensus.py` 脚本，执行时切勿引用不存在的文件。

### 4.4 面向 LAB/MEASURED 的最小工程增量

在正式进入多卡协同与零错误消费验收前，必须补齐以下工程增量：

1. **语义校验接口补齐**：为 `ConsumeEligibility` 补齐部分挂载计划、Generation 代际校验及协议版本接口；
2. **跨进程共享内存实现**：将 Ready 标志位、Consensus Epoch 及 8-bit 原子位图挂载至真实的 `/dev/shm` POSIX 共享内存，配备严密的原子内存序；
3. **多进程 Rank 编排启动器**：开发支持拉起 8 个独立物理进程的启动器，实现独立日志记录、Rank ID 绑定及统一配置下发；
4. **协同决策与通信对接**：使共识决策真正驱动下游的缓存加载与重算分支，并对接真实的底层集合通信同步屏障；
5. **全场景故障注入工具**：实现针对慢卡超时、进程突发崩溃、Epoch 错位及协议版本冲突的自动化注水工具；
6. **全链路原始事件归档**：实现基于全量逐次原始样本的 P50/P95/P99 统计工具，完整留存错误消费与死锁监控日志。
...
- **操作意图**：确保所有评测数据均可精准追溯至唯一的源码版本、用例集与测试环境。
- **执行命令**：
...
- **应观察现象**：结果目录创建成功，Git Commit 哈希与工作区状态完整落盘。
- **判定边界**：若存在未记录的源码修改，测试结论最高判定为 `LAB`；源码版本无法确定时直接判定为 `INVALID-EVIDENCE`。

### 步骤 1：编译并运行当前 DEMO 确认边界

- **操作意图**：验证当前 C++ 代码可编译构建，并将单进程 DEMO 的输出规范归档，避免将其误判为多卡实测。
- **执行命令**：
...
- **应观察现象**：终端输出平均校验耗时与模拟同步时延；导出的 CSV 明确标记 `DEMO,DEMO_ONLY`。
- **判定边界**：本步骤仅证实编译与输出 Schema 正常，不可据此断言 TP=8 多卡共识或分歧回退已经成立。

### 步骤 2：运行未接入 SUT 的 Harness 确认防伪造边界

- **操作意图**：验证测试脚手架在未接入被测适配器时能够显式报告未执行，杜绝将空执行伪造为测试通过。
- **执行命令**：
...
- **判定边界**：若分析报告将该未执行结果误判为 8 类用例测试通过，报告本身判定为 `INVALID-EVIDENCE`。

### 步骤 3：接入最小 SUT 适配器执行 C01~C06 正确性实测（条件步骤）

- **前置条件**：已开发并接入基于 stdin/stdout JSON 协议的被测适配器，完成输入字段到 `SemanticMetadata` 的精准映射。
- **操作意图**：使每一个基础语义冲突用例均真实调用底层校验引擎，并由 Harness 独立对账。
- **执行动作**：为 C01~C06 分别生成正向与负向样本，传入被测适配器命令执行测试：

```bash
python3 ./test_correctness.py   --sut-command "./consume_eligibility_adapter"   --out "../../../../results/pvt06/${run_id}/correctness_c01_c06.json"
```

- **应观察现象**：每一个测试用例的 `actual.status` 均与预设的 `expected` 严格吻合；任何适配器报错、非 JSON 输出或非零退出码均被真实捕获。
- **判定边界**：在未接入真实被测适配器前，本步骤统一规范记录为 `NOT-SUPPORTED`。

### 步骤 4：补齐并执行 C07/C08 协议扩展用例（条件步骤）

- **操作意图**：验证长 Prompt 部分前缀挂载与单卡分歧协同回退的高阶协议动作。
- **执行动作**：针对 C07 验证匹配前缀与冲突尾部的拆分计划；针对 C08 验证单卡分歧下 8 个 Rank 是否均获得同一协同回退决策；由独立 Oracle 严格对账。
- **应观察现象**：C07 绝不会将部分匹配误判为全量命中；C08 下全量 Rank 均输出带有相同 Epoch 的协同重算决策。
- **判定边界**：当前受控 C++ 原型尚未具备此能力；在未完成工程扩展前，严禁将 C07/C08 标记为通过。

### 步骤 5：运行真实多进程 TP=8 状态同步（条件步骤）

- **前置条件**：`world_size=8` 多进程编排器已就绪；`/dev/shm` POSIX 共享内存已打通；独立日志采集环境已搭建。
- **操作意图**：在真实 8 个独立物理进程中验证原子位图状态发布、并发读取、决策收敛及后续集合通信。
- **执行动作**：拉起 8 个独立 Rank 进程，采集各卡共识开始/完成时间戳、状态决策、超时事件、进程退出码及共享内存资源释放记录。
- **应观察现象**：全命中时 8 卡同步进入加载；任一卡发生校验拒绝或超时时，8 卡统一步调一致回退至本地重算；测试结束后共享内存 0 残留。
- **判定边界**：单进程运行或使用线程局部变量的测试，一律不可纳入 TP=8 `MEASURED` 生产级汇总。

### 步骤 6：全场景故障注水与集合通信死锁防护（条件步骤）

- **操作意图**：确证协同回退与集合通信在发生单卡网络抖动、进程崩溃或版本错位时，绝不会引发集群死锁。
- **执行动作**：分别向 Rank 7 注入语义拒绝、超时延迟、突发终止进程、旧 Epoch 错位及协议版本冲突；观测其余存活 Rank 是否能在明确超时后安全收敛并退出。
- **应观察现象**：分歧发生时绝不会出现部分卡加载、部分卡重算的分裂状态；集合通信算子要么按统一回退路径平稳完成，要么返回明确错误码，绝对杜绝无限期永久挂起。
- **判定边界**：若缺乏底层集合通信调用或同步屏障凭证，不可宣称已实现死锁防护。

### 步骤 7：全链路统计、独立复核与证据归档

- **操作意图**：基于全量原始逐次样本精确计算时延分位数与正确性指标，杜绝任何静态常数或平均值掩盖长尾波动。
...
- **应观察现象**：摘要报告中的每一个 P99 时延与正确性结论均能精准追溯至底层原始日志。
...
约束说明：`wrong_consume = false` 仅在被测操作真实执行、校验拒绝且未发生后续数据接入时方可填报；若 SUT 未实际执行，该字段必须填报为 `null`，严禁用 `false` 替代未知。
...
事件至少包含：`eligibility_done`、`state_publish`、`consensus_read`、`decision`、`collective_enter`、`collective_done`、`cleanup` 与 `process_exit`。
...
| 多卡决策绝对一致性 | C08 及全场景故障注水下全卡决策分歧数严格 $= 0$ | 同一 Epoch 下全量 8 个 Rank 均具备自洽日志凭证 |
| 6 维语义校验耗时 | 单次 6 维语义校验耗时 P99 $< 5\mu	ext{s}$ | 全量逐次原始采样，固定 CPU 绑核与编译优化 |
| 多卡状态同步耗时 | TP=8 多卡状态同步耗时 P99 $< 100\mu	ext{s}$ (RankConsensus) | 真实跨进程共享内存或底层通信栈实测，严禁使用固定常数 |
| 集合通信死锁发生率 | 全场景故障注水下集合通信死锁数严格 $= 0$ | 进程树监控、通信完成中断日志或显式异常返回 |
| 陈旧脏数据错误消费 | 全量测试场景下陈旧失效数据错误消费数严格 $= 0$ | 具备正文数据实际消费动作与 Oracle 的严格逐条对账 |

### 7.2 状态判定枚举与规则

- **GO（全链路证据形成完整闭环）**：基础正确性、TP=8 多卡共识、故障协同回退及微秒时延门限均具备完整 `MEASURED` 证据链；
- **CONDITIONAL（局部拓扑或参数达标）**：正确性校验通过，但仅在特定拓扑、受限规模或特定超时配置下成立；结论严格限定于已测条件；
- **NO-GO（发生错误消费或死锁）**：发生陈旧数据错误消费、Rank 决策出现分歧、集合通信发生死锁或出现未受控超时；
- **NOT-SUPPORTED（物理环境未支持）**：现场缺乏 TP=8 硬件加速卡、跨进程共享内存、SUT 适配器或底层集合通信环境；
- **INVALID-EVIDENCE（无效证据）**：用例未实际执行、Oracle 缺失、静态常数冒充实测、时间线不完整或空值被错误写为 0。
...
- 任意 Rank 发生无限等待、进程无法正常退出或共享内存残留资源不断累积泄漏；
- 旧 Epoch、旧代际 Generation 或未 Ready 的半写脏数据被下游算子读取消费；
- 测试脚手架捕获到 SUT 异常报错却依然将其统计为拦截成功；
- 底层集合通信库抛出未恢复的严重通信错误或加速卡设备上下文异常崩溃。
...
|---|---|---|---|
| E0（契约与用例规范确认） | 严密核对 C01~C08 用例定义、结果枚举、Oracle 判定逻辑及无效证据规则 | 源码审计报告、Harness 架构图、DEMO 输出 JSON | 确认当前代码边界，杜绝将测试桩冒充实测 |
...
- **正确性测试结果显示 `status=NOT_EXECUTED`**：说明未向脚本传入 `--sut-command` 参数；需接入可审计的被测系统适配器，未接入前严格保持为无效或未支持状态。
- **C07 或 C08 用例执行始终无法通过**：当前受控 C++ 源码中尚未实现 Partial Attach 拆分或多卡协同协议；需先补齐底层接口与状态机模型，严禁通过篡改 expected 字符串伪造通过。
- **运行命令传入 `--ranks` 或 `--out` 未改变程序输出**：当前受控基准程序未集成 CLI 参数解析；应记录为 `DEMO` 并在重构中补齐参数解析与独立输出路径。
- **测得的共识耗时 P99 严格等于某一固定常数**：说明代码中仍在使用 `+18.5` 静态偏移模拟同步开销；必须剔除静态常数，采集真实的跨进程共享内存同步事件。
- **执行测试提示 `/dev/shm` 无法复用或残留报错**：排查共享内存所有权管理、超时自动清理及进程异常退出时的清理 Handler，确保测试前后资源 0 残留。
- **注入单卡分歧后其余存活 Rank 依然执行缓存加载**：说明多卡共识决策未能真正驱动下游分支执行；必须将全卡一致决断作为唯一的准入分支门。
- **租约时间戳与当前时间完全相等时被放行**：核对代码中是否采用了 `>` 判断而非 `>=`；需在协议层面固化边界定义并补充对应的回归测试用例。
- **测试报告将未执行用例统计为 0 错误消费**：排查 `wrong_consume_count` 的空值处理逻辑；未实际执行的用例该字段必须保持为 `null`，汇总判定为 `INVALID-EVIDENCE`。
- **测试中无死锁发生但亦未捕获到集合通信事件**：说明测试仅执行了上层位图决策，未实际调用底层通信算子；该测试点仅能证明共识逻辑，不可据此断言集合通信绝对安全。
