...
> **公共执行契约**：本项遵循 [Benchmark 公共契约与证据分级规范](./Benchmark公共契约与证据分级规范.md)。每个测试 `run_id` 需在运行前完整固化 DPU/网卡/NPU 硬件型号、驱动固件版本、PCIe/NUMA 物理拓扑、传输通道模式、Payload 数据量、加密算法与完整性校验策略、硬件队列深度、并发请求数、故障注入参数、代码包版本、配置哈希及目标证据等级；实测产出必须完整保留描述符提交/硬件完成中断/CQE 事件、超时监测日志、故障隔离动作、通道切换确认凭证、端到端请求结果、实测带宽/时延、Host CPU 占用率、Host Payload Touch 探针采样、网络丢包统计、数据完整性校验及底层硬件错误码。在现场环境缺乏特定硬件能力时，需标记为 `NOT-SUPPORTED`，严禁使用静态模拟数据替代。

> **验证范围声明**：在当前受控的原型验证工程中，`offload_fallback_bench.cc` 仅解析 `--hardware-supported` 与 `--out` 参数，并在输出 CSV 中固定写入 Raw Direct、DPU 及故障回退的演示数据行；`inject_fault.py` 仅在等待固定时间后生成独立的故障描述 JSON，并未实际启动被测系统 (SUT)、未控制物理设备驱动，亦未精确测量故障检测与切换时延。现有受控源码尚未打开 DPU、网卡或 NPU 设备句柄，未执行真实的 AES-GCM 加密、CRC 校验、硬件 DMA 传输、CQE 完成事件监听、看门狗心跳或生产级故障回退。因此，现有受控源码仅用于检验字段结构与故障流转流程预演，不可直接作为证明吞吐性能、Host CPU 卸载收益、数据完整性、500µs 切换或请求连续性保障的依据。在缺乏物理设备 I/O、真实故障时间线及数据一致性校验的前提下，测试结论统一限定标记为 `DEMO` 或 `LAB`；若业务安全策略强制要求正文加密，而 Raw Direct 缺乏等价密码学硬件支持，该路径需标记为 `NOT-SUPPORTED` 或显式安全拒绝。
...
> **开源基线版本**：Mooncake `f90ae691f109e49a60920e0c8abbf7e572826d8c`。正式测试结果需绑定实际 DPU/网卡/NPU 硬件设备、驱动固件版本、加密算法库及配置哈希。
...
在分布式大模型推理架构中，KVCache 正文数据的网络传输与多级存储沉淀面临安全与合规约束。加密计算（如 AES-256-GCM / SM4）、消息认证码（MAC/Tag）生成以及数据完整性校验（CRC64/T10-DIF）会消耗 CPU 与加速卡算力。将此类安全与数据转换任务卸载至 DPU 专用协处理器，能够释放主机 CPU 与 NPU 算力专注于在线推理；然而，DPU 卸载同时引入了专有硬件固件、驱动栈、硬件队列及潜在的单点故障等外部依赖。

相对而言，Raw Direct 原始直达路径仅依赖基础的网卡与 NPU DMA 控制器，不依赖外部专用协处理器，系统复杂度低且运行稳定。因此，本方案明确了“**以 Raw Direct 纯软直达为主路径底座，以 DPU 硬件卸载为条件加速轨**”的软硬双轨协同体系。

在双轨协同架构中，故障回退遵循清晰的控制链：
...
(检测到 CQE 超时 / 心跳丢失)
        │
        ▼
触发熔断器 (Circuit Breaker) ──► 阻断新请求提交至故障队列
        │
        ▼
...
若在途请求已部分写入目标显存缓冲区，回退机制需识别有效写入边界，杜绝将未完成的半写块直接标记为 `READY`。同时，AES-GCM 的认证 Tag 校验与 Generation 代际版本需深度参与判决，防止因硬件故障引发脏数据被错误消费。

### 0.2 Host CPU 零数据拷贝与数据面物理边界

系统设计了微秒级控制面与零拷贝数据面的双流分离架构：
...
Host CPU 负责管理会话密钥句柄、安全策略、通道状态及错误码，但不将 KVCache 正文数据搬运至 Host DDR 执行 `memcpy`、软件 CRC 或软件加密。Host Payload Touch 探针需完整覆盖正常提交、硬件完成、异常重试及故障回退的全生命周期。
...
|---|---|---|
| DPU 硬件能力实测 | DPU 硬件是否支持目标 AES-GCM/SM4 加密、Tag 认证、CRC64 校验或格式转换 | 运行时硬件能力矩阵、固件驱动版本、真实 I/O 及数据校验记录 |
| Raw Direct 独立性 | 在无 DPU 协处理器的纯软环境下，系统能否独立稳定完成端到端数据传输 | 真实的 NPU HBM 显存目标地址、DMA/CQE 完成事件、数据校验及零拷贝证明 |
| 算力卸载真实收益 | 在相同安全策略与负载下，DPU 卸载能否降低 Host CPU 占用并提升有效吞吐 | 逐 I/O 采样样本、CPU/设备性能计数器、严格同安全策略下的 A/B 对照 |
...
| Raw Direct 独立基准 | 无 DPU 环境下 Raw Direct 独立完成传输，吞吐达标 | 与同安全策略下的测试基线比对 |
...
| Host CPU 正文拷贝 | 正常直达路径下 Host Payload Touch 严格 $= 0$ | 由动态探针或硬件性能计数器全程证明 |
...
| **条件 D** | **注入硬件超时故障** | **自动触发回退** | 严格维持合规安全策略 | **故障熔断与安全回退实测** |
...
| 分片颗粒度 (Chunk) | 64KB、1MB、4MB | 遵循底层硬件 DMA 与网络 MTU 对齐约束 |
| 硬件队列深度 (QD) | 1、4、16、32 | 评估不同并发队列下的设备吞吐与排队时延 |
| 数据流向维度 | 网卡到 HBM、HBM 到网卡 | 覆盖换入与换出双向数据通路 |
...
6. **全链路密码学与数据校验**：集成端到端 AES-GCM Tag 认证、CRC64 检验及篡改样本注入测试。
...
- **操作意图**：确保对比测试均可精准追溯至唯一的源码版本、硬件拓扑及安全合规策略。
- **执行命令**：
...
- **应观察现象**：输出 CSV 包含演示数据行，标记 `DEMO,DEMO_ONLY`。

### 步骤 2：运行当前故障注入脚本确认事件格式

- **操作意图**：验证故障事件的 JSON 结构。
- **执行命令**：
...
- **执行动作**：导出 PCIe 拓扑与设备固件版本；探测 DPU 支持的 AES-GCM/SM4/CRC64 算法及 HBM 地址映射能力；生成 `hardware_capability_matrix.json`。
...
- **操作意图**：确证 DPU 发生硬件超时或崩溃时，系统能在短时间内平稳熔断并安全回退至 Raw Direct。
...
- **GO（全链路证据闭环）**：通道能力、数据正确性、零拷贝验证、吞吐性能及故障回退均具备完整 `MEASURED` 证据链；
- **CONDITIONAL（局部场景或特定硬件达标）**：核心门限在特定硬件配置或特定安全策略下达标，但特定异常下存在波动；
...
|---|---|---|---|
| E0（契约与策略规范确认） | 固化安全策略、能力矩阵 Schema、通道枚举、回退逻辑及证据等级规范 | 源码审计报告、DEMO 输出 CSV/JSON、规范化 Manifest | 确认当前代码边界，杜绝将测试桩冒充实测 |
...
| E3（故障回退与双轨混压总门禁） | 注入 CQE 超时与心跳丢失故障，全量验证看门狗检测、熔断隔离与极速切换 | 故障时间线分析表、切换确认日志、最终判定结论 | 证实切换时延 $< 500\mu\text{s}$ 且 0 错误消费 |
| 条件证伪（DPU 依赖与安全边界） | 分别关闭 DPU、关闭在途转换，证伪软硬双轨协同的必要性与技术边界 | 双轨依赖评估表、降级安全合规报告 | 确立纯软主路径，若硬件不支持则规范标记未支持 |
...
- **AI Agent 协同职责**：
  ...
  4. 严守技术诚信红线，严禁虚构 DPU 吞吐、伪造切换时延或将静态演示数据篡改为生产级实测。
...
- **输出 CSV 中吞吐与时延显示为固定的 685/660 样例数值**：当前受控 C++ 代码仅为演示 DEMO；必须接入真实的底层驱动与硬件性能计数器。
- **命令行传入 `--mode` 或 `--payload-mb` 报错未知参数**：当前受控基准程序未集成此类 CLI 参数；应按照实际支持的 CLI 参数执行。
- **运行故障注入脚本传入 `--sut-cmd` 报错未知参数**：当前脚本不包含被测进程编排逻辑；需由外部测试脚手架统一编排。
...
- **Raw Direct 传输成功但业务安全策略报警拦截**：当前网络环境强制要求密码学加密而 Raw Direct 未配备硬件加密引擎；应规范判定为 `NOT-SUPPORTED` 或显式安全拒绝。
