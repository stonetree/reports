...
> **公共执行契约**：本项遵循 [Benchmark 公共契约与证据分级规范](./Benchmark公共契约与证据分级规范.md)。每个测试 `run_id` 需在运行前完整固化内存分配器类型、底层加速硬件、CPU/NUMA 绑核拓扑、页迁移调度策略、并发 Reader 读取线程数、迁移物理块尺寸 (Block Size)、迁移循环轮次、读写并发比例、故障注入点、代码包版本、配置哈希及目标证据等级；实测产出必须完整保留 Reader 进入/退出临界区日志、指针原子读取与 CAS 翻转时间戳、NPU Stream/DMA 事件、双层宽限期判定、旧内存块释放事件、xxHash64 校验码、Generation 代际版本、异常安全回滚轨迹、前台在线停顿样本及 TPOT 尾部时延。现场缺乏硬件 Atomic Remap 能力时需标记为 `NOT-SUPPORTED`，严禁填入静态模拟数据；证伪结论需限定在已验证的访问契约、底层硬件拓扑及显存迁移场景之内。
...
> - **Checksum**：用于检测数据迁移前后一致性的密码学/哈希校验码；
...
> **开源基线版本**：Mooncake `f90ae691f109e49a60920e0c8abbf7e572826d8c`。正式测试结果需绑定实际显存分配器、NPU 运行时驱动、硬件事件日志及配置哈希。
...
在长时间高并发的大模型推理服务中，显存频繁的分配与释放会导致显存碎片化，进而降低长上下文请求的并发承载能力。显存页迁移（PageMigration / Defrag）的核心目标是在线整理显存碎片，其物理安全性需在底层闭环以下三个关键操作：

```text
1. 异步复制与校验 : 将旧物理区间 (OldExtent) 数据完整复制至新分配的物理区间 (NewExtent)，并校验新数据的一致性；
2. 原子指针翻转   : 通过微秒级原子操作 (CAS) 将活跃指针切换至 NewExtent，使后续新进入的 Reader 能够立即无锁访问新显存块；
3. 延迟安全释放   : 必须在确认全量持有 OldExtent 引用的 Host 线程与 NPU 异步硬件算子全部退出后，方可回收释放 OldExtent。
```

在系统底层，指针翻转耗时达到微秒级并不代表旧显存块已可安全释放；统计 Host 侧的 CPU 线程引用，也无法代表 NPU 硬件流水线中的异步 Attention 算子已停止读取旧显存物理地址。若在 NPU 异步算子尚未执行完毕前过早释放旧显存块，会引发 **Use-After-Free (UAF)** 内存越界错误或推理计算输出乱码。

软件 RCU 机制的核心，就是建立“**Host CPU 临界区追踪 + NPU Stream Event 硬件流事件**”双层宽限期仲裁协议，将旧块释放推迟至所有硬件引用清空之后，从而在纯软层面降低前台停顿并保障内存安全。
...
| 真实实验现象 | 科学严密支持的技术结论 | 结论适用边界说明 |
|---|---|---|
| 软件 RCU 真实实测全量通过 | 在已验证的硬件拓扑、驱动版本及双层引用协议下，纯软 RCU 方案满足微秒级停顿与数据安全门限，证明在此场景下**无需额外依赖专有硬件 Atomic Remap 原语** | 限定在已验证的硬件与拓扑范围内，不作无边界外推 |
| 软件 RCU 实测发生数据越界或长尾 | 说明当前软件双层宽限期或 NPU 事件监听实现存在工程缺陷，需修复或评估硬件原语引入 | 客观反映工程现状，不盲目下结论 |
| 硬件 Atomic Remap 现场不支持 | 确认现场硬件与驱动环境暂不具备对应硬件原语，规范标记为 `NOT-SUPPORTED` | 在缺乏硬件对比的前提下不宣称软硬件性能对比 |
...
| 显存延迟释放安全 | 旧 Extent 显存块是否在全量 Host Reader 与 NPU 异步算子彻底退出前保持有效 | 原子引用计数、NPU Stream Event 硬件中断及显存释放时间戳对账 |
| 在线停顿削减收益 | 相比传统的全局暂停 (Stop-the-World)，软件 RCU 方案能否消除前台读者的阻塞停顿 | 全量逐次停顿时间采样样本、P50/P95/P99 统计分布 |
...
| 硬件依赖必要性证伪 | 在已打通的双层宽限期契约下，纯软方案是否已满足性能与安全门限 | 软件 `MEASURED` 证据链、硬件探测日志及客观技术边界声明 |
...
| 前台在线 TPOT 干扰率 | 显存页迁移期间前台推理 **P99 TPOT 干扰恶化率 $< 3\%$** | 基于同一增强代码包在无迁移干扰下的纯前台基线对账 |
...
面向跨设备内存整理与安全管理，底层 Extent 描述符标准化定义：
...
4. 数据一致性校验 : 计算并核验 NewExtent 的 xxHash64 校验码与原始数据一致
...
9. 标记并安全释放 : 将 OldExtent 状态置为 RETIRED 并回收物理显存块
```
...
| 迁移测试重复轮次 | 100 轮起步，正式目标 1000 轮 | 充分捕获 P99 尾部时延与内存时序竞争 |
| 读/写并发比例 | 纯读并发、读写 10:1、读写 1:1 | 模拟多轮对话下的显存分配与访问交织压力 |
| 故障点注入场景 | 拷贝后翻转前故障、翻转后宽限期内故障、释放前故障、Checksum 校验失败、NPU Event 超时 | 验证系统在各种时序断点下的自愈与安全回滚能力 |
...
- **操作意图**：确保对比测试均可精准追溯至唯一的源码版本、分配器配置及硬件驱动环境。
- **执行命令**：
...
- **应观察现象**：导出的 CSV 文件包含三类方案的演示数据行，标记 `DEMO,DEMO_ONLY`。

### 步骤 2：运行真实并发 Reader 与软件 RCU 最小闭环（条件步骤）

- **操作意图**：在 Host 多线程并发环境下，证实软件 RCU 读临界区、异步拷贝、原子 CAS 翻转及延迟释放的逻辑正确性。
...
- **操作意图**：将 NPU 异步算子与底层 DMA 硬件生命周期纳入宽限期仲裁。
- **执行动作**：在 NPU 上提交引用旧 Extent 的异步算子；在指针翻转前后插入硬件流事件标记；后台迁移线程等待 Host 读者清零并等待 NPU Stream Event 完成中断后释放旧显存块。

### 步骤 4：运行 Stop-the-World vs 软件 RCU 同场景 A/B 实测（条件步骤）

- **操作意图**：在相同的并发负载与显存压力下，对比两种方案的停顿时间、前台 TPOT 干扰率及数据正确性。
- **执行动作**：依次运行 `STOP_THE_WORLD` 强同步方案与 `SOFTWARE_RCU_COPY_ON_MIGRATE` 软件 RCU 方案；采集前台 Reader 停顿时间样本及在线推理 TPOT 序列；计算 P50/P95/P99 时延分位数。

### 步骤 5：全时序断点故障注入与安全回滚实测（条件步骤）

- **操作意图**：确证软件 RCU 机制在各种异常扰动下不会发生提前释放、内存泄漏或脏块放行。
- **执行动作**：在拷贝完成、CAS 翻转后、宽限期等待中及校验失败等断点分别注入异常；观测系统是否平稳拦截并触发安全回滚；持续运行 Reader 检验是否存在非法内存访问。

### 步骤 6：现场硬件 Atomic Remap 能力探测与对照（条件步骤）

- **操作意图**：客观探测现场硬件与固件是否支持原子重映射原语；若支持则开展同场景对照，不支持则记录 `NOT-SUPPORTED`。
...
$$
\text{前台在线 TPOT 干扰率} : \quad \Delta \text{TPOT}_{\text{P99}} = \frac{\text{TPOT}_{\text{P99}}(\text{Migrating}) - \text{TPOT}_{\text{P99}}(\text{Baseline})}{\text{TPOT}_{\text{P99}}(\text{Baseline})} \times 100\%
$$

$$
\text{显存安全释放延迟裕量} : \quad \Delta T_{\text{safe}} = T_{\text{release}}(\text{OldExtent}) - \max\left( T_{\text{exit}}(\text{LastHostReader}), T_{\text{done}}(\text{NPUStreamEvent}) \right) \ge 0
$$
...
- **GO（科学证伪证据形成闭环）**：在明确的设备引用、分配器及拓扑范围内，软件 RCU 的停顿、TPOT 干扰率、数据正确性及回滚安全性均具备完整 `MEASURED` 证据链；可严密表述为“在已验证条件下不需要硬件 Atomic Remap 强依赖”；
- **CONDITIONAL（局部场景或特定规模达标）**：软件方案在特定并发或特定块大小下达标，但扩展至超大块时存在波动；
- **NO-GO（发生内存越界或数据损坏）**：出现提前释放、UAF 越界访问、脏读、旧代次消费或回滚失败；
- **NOT-SUPPORTED（物理环境未支持）**：现场缺乏硬件 Atomic Remap 原语或缺少 NPU 硬件流事件接口；
...
|---|---|---|---|
| E0（命题与契约规范确认） | 固化证伪命题边界、Manifest Schema、双层宽限期定义及证据等级规范 | 源码审计报告、DEMO 输出 CSV/JSON、规范化 Manifest | 确认当前代码边界，杜绝将测试桩冒充实测 |
...
- **AI Agent 协同职责**：
  ...
  4. 严守技术诚信红线，严禁虚构停顿指标、伪造硬件重映射支持或将静态演示数据篡改为生产级实测。
...
- **输出 CSV 中停顿与时延显示为固定的 0.08ms/3.8µs 样例数值**：当前受控 C++ 代码仅为演示 DEMO；必须接入真实的并发 Reader 线程与显存迁移引擎。
- **命令行传入 `--mode`、`--readers` 或 `--loops` 毫无效果**：当前受控基准程序未集成此类 CLI 参数；应按照实际支持的 CLI 参数执行。
- **执行命令提示找不到 `verify_checksum.py` 脚本报错**：当前受控工程中尚未包含该脚本；需补齐校验器实现或人工核验。
- **Host 读者全部退出但 NPU 异步算子发生显存越界崩溃**：说明仅等待了 Host 宽限期而未将 NPU Stream Event 纳入仲裁；必须接入硬件流事件监听，在完成中断上报前严禁释放旧显存块。
- **CAS 指针翻转成功后数据 Checksum 校验失败**：排查数据拷贝完成与指针翻转的时序依赖；必须在 Checksum 100% 校验成功后方可执行指针翻转。
- **注入故障后进程未崩溃但数据输出异常**：必须严格核验 xxHash64 校验码、Generation 代际及显存释放时间戳。
...
- **旧显存块释放时间戳早于 NPU Stream Event 完成时间戳**：说明双层宽限期逻辑存在漏洞；必须立即终止测试，判定为一票否决的 `NO-GO` 并修复释放协议。
