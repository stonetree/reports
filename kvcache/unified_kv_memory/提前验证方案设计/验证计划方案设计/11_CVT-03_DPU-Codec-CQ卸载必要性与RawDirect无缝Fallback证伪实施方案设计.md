# CVT-03：DPU-Codec-CQ 卸载必要性与 RawDirect 无缝降级证伪实施方案设计

> **验证 ID**：CVT-03  
> **验证名称**：DPU / 硬件 CQ 路由 / 硬件 Codec 卸载必要性与 Raw Direct 路径无缝降级证伪  
...
1. **优先证伪必需性**：在完全没有 DPU / Codec 专用硬件加速卡时，存储池主路径基于 **Raw Direct（原始直达路径：不依赖 DPU 协处理器与专用硬件压缩，仅依靠底层网卡/SSD DMA 直达的最小基础数据通路）** 通路完全独立成立，有效传输带宽达到物理网络线速的 **$\ge 80\%$**，且 Host CPU 占用率 **$< 1\%$**；
2. **无缝降级 (Fallback) 验证**：在配置了 DPU / 硬件 Codec 的环境下，人为注入 DPU 驱动挂死或硬件通道中断，系统能够在 **$< 1\text{ms}$ 内 $100\%$ 无缝降级切换至 Raw Direct 路径**，业务零报错、无断流；
