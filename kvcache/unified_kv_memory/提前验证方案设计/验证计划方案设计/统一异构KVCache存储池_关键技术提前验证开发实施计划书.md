...
### 1.1 研发立项战略背景与使命
统一异构 KVCache 存储池是面向下一代大规模长文本推理集群的关键基础设施。根据立项初评意见，本项目明确采用**“基于开源 Mooncake 全面魔改与深度重构”**的研发路径。在正式立项与全面铺开系统级工程前，开展提前技术原型验证（PVT-00 ~ PVT-07 必做包与 CVT-01 ~ CVT-03 条件证伪项）的核心使命是：
**“以 Mooncake 开源底盘为基础，以代码重构为抓手，以实测为准绳，全面摸清底层软硬件技术边界，消除架构设计模糊性，以客观实测数据为依据，为后续系统顺利落地提供扎实的技术支撑。”**

研发团队绝不做“概念演示”，必须将方案中的 9 大标准化工程模块、4 大维度 18 项工程约束在物理硬件上 100% 落地闭环，并构建原生 Mooncake 与魔改增强版的严格消融对比实验。

### 1.2 总体实施目标与四个核心验证阶段 (E0~E3) 映射
整个开发实施过程以**四个核心验证阶段与目标 (E0~E3)**为里程碑检验标准，确保每一行代码、每一次测试都对准核心技术目标：

```
┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
│                                四个核心验证阶段 (E0 ~ E3) 与 11 项验证包映射全景图                      │
├──────────────┬────────────────────────┬────────────────────────────────┬─────────────────────────┤
│ 验证阶段     │ 核心命题 / 准出门槛    │ 包含验证包                     │ 关键量化判定标准        │
├──────────────┼────────────────────────┼────────────────────────────────┼─────────────────────────┤
│ E0 收益确认  │ 验证 Saved-Prefill 净  │ PVT-00 (Saved-Prefill上限)     │ 净收益 ≥ 2.0× 总开销;   │
│              │ 收益是否真实存在       │                                │ UBMEM 加速比 ≥ 1.3×     │
├──────────────┼────────────────────────┼────────────────────────────────┼─────────────────────────┤
│ E1 核心路径与│ 验证零HostTouch传输、  │ PVT-01 (零HostTouch底座)       │ CPU触碰严格=0,线速≥80%; │
│ 多卡状态同步 │ DAG重叠与0错误消费     │ PVT-02 (Layout编译器/DAG)      │ 描述符压缩≥50%,重叠≥60%;│
│              │                        │ PVT-06 (6维校验与多卡共识)     │ 冲突消费=0,TP8共识<100us│
├──────────────┼────────────────────────┼────────────────────────────────┼─────────────────────────┤
│ E2 动态调度与│ 验证微秒决策优越性与   │ PVT-03 (DirectView边界/ViewGuard) 确认临界点,证伪DecodeView; │
│ 分层存储扩容 │ HBM-SSD 容量扩展主路径 │ PVT-04 (QueryPlan微秒决策引擎) │ P99<5us,准确≥90%,负利<1%;│
│              │                        │ PVT-05 (NVMe直达容量主路径)    │ 150%超载容量+30%,OOM-50%│
├──────────────┼────────────────────────┼────────────────────────────────┼─────────────────────────┤
│ E3 混压总门禁│ 验证全链路前后台混压下 │ PVT-07 (端到端混压最小闭环)      │ TTFT降≥20%,QPS升≥10%,   │
│              │ 的 QoS 服务质量保障 (QoS)        │                                │ TPOT 抖动干扰率 < 3%    │
├──────────────┼────────────────────────┼────────────────────────────────┼─────────────────────────┤
│ 条件证伪阶段 │ 证伪硬件多播、硬件Remap│ CVT-01 (热点前缀软件Fanout)    │ 软件Fanout时延差距<10%; │
│ (架构简化)   │ 与 DPU 硬件卸载必需性  │ CVT-02 (软件RCU PageMigration) │ RCU停顿<1ms,TPOT抖动<5%;│
...
├────────────────────────────────────────────────────────────────────────────────────────┤
│                          【开发组长 / Tech Lead】 (P8)                                 │
│  - 总体技术把控、架构决策、4个核心验证阶段与18项约束闭环、外部资源协调、E3全链路前后台混压总门禁总负责     │
├──────────────────────────┬──────────────────────────┬──────────────────────────────────┤
...
| **PVT-01** | Host CPU 零数据拷贝传输底座 | **R** | I | C | C | **A** |
...
| **PVT-07** | 端到端混压最小闭环总门禁 | C | C | C | **R** | **A (主推)** |
...
| **PVT-06** | xxHash64 6维校验、TP=8 /dev/shm 多卡状态同步 | 6 人日 | Dev-3 | `consume_eligibility`、8大冲突拦截与共识时延表 |
| **PVT-07** | 2节点全链路串联、前后台混压、QoS 服务质量保障 (QoS) | 8 人日 | QA/全员 | 端到端最小闭环报告、TTFT/TPOT 服务质量保障 (QoS)图 |
...
| **Phase 4** | 11份标准报告汇总、证据包打包、立项答辩准备 | 4 人日 | 组长/QA | 《关键技术提前验证证据包 (技术验证证据包 (Evidence Pack) V1.0)》 |
...
gantt
    ...
    原厂 liburma/libubmem SDK 头文件打通:m0_3, 2026-08-19, 2026-08-22
    ...
    PVT-07 前后台混压、QoS 限速与时延稳定性测定:crit, p3_2, 2026-09-14, 2026-09-17
    ...
    立项证据包 技术验证证据包 (Evidence Pack) 终审归档   :p4_2, 2026-09-18, 2026-09-19
...
│                                      │                                                 │
│                                      ▼                                                 │
│                   [立项证据包打包与 Go/No-Go 终审 (技术验证证据包 (Evidence Pack))]                     │
│                                                                                        │
└────────────────────────────────────────────────────────────────────────────────────────┘
```
...
- 两条主线在 **PVT-07** 汇聚，构成不可分割的立项前最小闭环系统。
...
│ **WBS 8.0**  │ WBS 8.1 ~ 8.5     │ **PVT-07** 前后台混压端到端最小闭环与 SemanticQoS 服务质量保障 (QoS)验证  │
...
- **核心判定门禁**：
  ...
  - $TP=8$ 多卡多卡状态同步耗时 $P99 < 100\mu s$；
  - 发生分歧时 8 卡 $100\%$ 协同回滚至本地重算，0 死锁，0 挂起。

---

### 5.8 PVT-07：前后台混压端到端最小闭环与 SemanticQoS 服务质量保障 (QoS)实施规程
...
- **WBS 任务拆解**：
  ...
  - `WBS 8.5`：导出全链路 TTFT、TPOT、QPS 与干扰率，生成 E3 全链路前后台混压总门禁判定结论。
- **核心判定门禁**：
...
│ 5. 终审证据包│ 《统一异构 KVCache 存储池 关键技术提前验证全量证据包 (技术验证证据包 (Evidence Pack))》  │
...
├── 08_PVT-07_前后台混压端到端最小闭环总门禁报告.md
...
---
> **开发组长总结**：  
> 本计划书是研发攻坚团队的实施指导手册。全员遵循 18 项工程对齐规范，恪守三条质量基线，按 WBS 分步推进，以客观实测数据填满每一个交付表格，保障统一异构 KVCache 存储池方案高质量顺利落地。
