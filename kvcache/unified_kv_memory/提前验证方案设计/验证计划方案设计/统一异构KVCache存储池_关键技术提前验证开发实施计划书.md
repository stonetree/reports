...
> **公共执行契约**：全部 PVT/CVT 统一遵循 [Benchmark 公共契约与证据分级规范](./Benchmark公共契约与证据分级规范.md)。设备和拓扑按实际可用环境参数化；每次结果必须声明 DEMO/LAB/MEASURED、W0/W1/W2 环境档位、完整代码包 ID 和配置哈希。
...
原型阶段以 Mooncake Commit `f90ae691f109e49a60920e0c8abbf7e572826d8c`、vLLM Commit `842dd8fd96650063e1ad32e6075742d457d39773` 和 vLLM-Ascend Commit `424e27e1fd2b1c6e0d7fe659b489b87c1223a33c` 为固定开发基线，建立官方 `benchmark_serving` 在线打流的完整工作流，再由开发人员结合可用设备完成真实数据路径接入。原生开源版与深度重构增强版使用独立代码包、配置和结果目录进行消融对账。

### 1.2 优先级 (P0 / P1 / P2) 与四个核心验证阶段 (E0~E3) 映射矩阵

```
┌──────┬────────┬─────────────────────────────────────────────────┬──────────┬──────────────────────────────────────────┐
│ 等级 │ 验证ID │ 验证项标准功能名称                              │ 对应阶段 │ 针对开源代码的核心重构与消融验证标准     │
├──────┼────────┼─────────────────────────────────────────────────┼──────────┼──────────────────────────────────────────┤
│      │ PVT-02 │ 异构框架 Layout 描述符编译器与跨节点异步流水    │ E1 阶段  │ C++ 描述符提交开销降≥40%, 流重叠率≥60%   │
│  🔴  │ PVT-04 │ 微秒级动态决策引擎与 CostEvaluator 成本模型     │ E2 阶段  │ 决策时延 P99<5µs, 准确率≥90%, 负收益率<1%│
│  P0  │ PVT-05 │ HBM-SSD 直达容量主路径与分层存储扩容            │ E2 阶段  │ io_uring FIXED 吞吐线速≥80%, OOM 降≥50%  │
│ 核心 │ PVT-06 │ 多维度语义一致性强校验与 TP=8 多卡状态同步      │ E1 阶段  │ 6维校验 0 错误消费, 多卡同步 P99<100µs   │
│ 决胜 │ PVT-07 │ 前后台混压全链路端到端总门禁与 SemanticQoS      │ E3 阶段  │ 三重消融总门禁: TTFT降≥20%, 前台干扰<3%  │
├──────┼────────┼─────────────────────────────────────────────────┼──────────┼──────────────────────────────────────────┤
│      │ PVT-00 │ 业务流量 Saved-Prefill 收益上限与加速边界评估   │ E0 阶段  │ 净收益≥2.0×总开销, UBMEM 加速比≥1.3×     │
│  🟡  │ PVT-01 │ Host CPU 零数据拷贝传输底座与硬件能力矩阵       │ E1 阶段  │ CANN P2P Pinning, eBPF CPU 触碰严格=0    │
│  P1  │ PVT-03 │ 远端直读 vs 显存拷贝适用边界与 ViewGuard 容错   │ E1/E2阶段│ 证伪 Decode 直读, SIGBUS 100% 安全回滚   │
│ 底座 │ PVT-09 │ DPU 硬件安全与算力卸载加速 vs Raw Direct 双轨验证│ E1 阶段  │ DPU 硬件线速 AES/CRC, 500µs 无缝降级     │
├──────┼────────┼─────────────────────────────────────────────────┼──────────┼──────────────────────────────────────────┤
│  🟢  │ PVT-08 │ 1-to-N 组播式 KV 分发：硬件多播 vs 软件分层中继 │ E1 阶段  │ 软件 Staging 组播相比单播源端带宽节省≥60%│
│  P2  │ CVT-01 │ 显存整理软件 RCU 机制 vs 硬件 AtomicRemap 验证  │ 条件证伪 │ 软件 RCU 停顿<1ms, 免除专用硬件芯片依赖  │
└──────┴────────┴─────────────────────────────────────────────────┴──────────┴──────────────────────────────────────────┘
```
...
gantt
    ...
    参数化物理集群、网络与SSD环境部署   :m0_2, 2026-08-19, 2026-08-22
    ...
    PVT-07 代表性跨节点全链路服务串联联调 :crit, p3_1, 2026-09-10, 2026-09-13
...
│              │                   │ • WBS 0.1: 按以下完整 Commit Hash 编译固定开源基线：           │
│              │                   │   Mooncake f90ae691f109e49a60920e0c8abbf7e572826d8c          │
│              │                   │   vLLM 842dd8fd96650063e1ad32e6075742d457d39773              │
│              │                   │   vLLM-Ascend 424e27e1fd2b1c6e0d7fe659b489b87c1223a33c       │
│              │                   │ • WBS 0.2: 按现场设备部署跨节点 URMA 与 NVMe SSD 环境         │
