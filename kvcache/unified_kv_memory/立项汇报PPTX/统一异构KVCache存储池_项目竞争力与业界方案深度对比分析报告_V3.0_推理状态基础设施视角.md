...
- **Mooncake、LMCache、NVIDIA KVBM/NIXL 等会继续向多层存储、意图感知、动态路径、状态管理和 SLO 决策收敛，但能力收敛不等于设计中心相同。** 如果本项目仅提供多级缓存、URMA、RDMA 或 SSD 直达，差异会被迅速抹平；如果项目把“状态价值、消费资格、加载与重算、实际路径证据、目标硬件反馈”固化成不可绕过的系统契约，则仍有独立成立价值。
- **本项目最具辨识度的技术品味，是把语义置于位置之前、把重算纳入资源层级、把 Ready 置于 Completion 之后、把实际路径置于宣传路径之上。** 系统不追求最大 raw hit 或最大缓存容量，而追求在截止时间内可安全消费且净收益为正的状态。
...
本报告在现有 `KVAccessIntent → QueryPlan → AttachHandle → ExtentManifest → Descriptor → Completion/Fence/Integrity → Ready` 契约链基础上，提出三个便于评审和后续设计讨论的概念：
...
## 十五、从 KVCache 到 AI State Fabric：长期机会与范围纪律

### 15.1 为什么 KVCache 可能是更大状态基础设施的第一站
...
- 发布 Connector/Provider ABI 和参考实现；
- 形成芯片、网络、SSD 联合认证；
- 建立客户工作负载画像、容量、状态价值与业务价值规划工具；
- 将生产证据反向输入下一代硬件设计。

**退出门槛**：至少两个生产级工作负载、两个推理运行时和两种数据平面完成可复现验证。

---

## 十九、评委汇报建议：用客户价值兑现世界观
...
Mooncake、LMCache、KVBM/NIXL 已经很强，但行业能力正在快速收敛。仅做多层缓存或新协议接入无法形成长期壁垒。仍未被统一解决的是跨层状态身份、净价值、消费资格、重算选择和实际路径责任。

#### 第三问：本项目到底交付什么？

交付的不是另一个 Store，而是一套状态契约和控制平面：识别一份 KV 是什么，估计它值多少，决定从哪里获得，证明它可以消费，并用实际结果持续校准软硬件系统。
...
先用四个决定性技术实验证明状态控制正确，再用客户 Trace、同条件基线和 BVT 证明技术收益能够稳定转化为客户业务回报。

### 19.2 建议的开场陈述

> 大模型推理正在从无状态计算走向状态化基础设施。KVCache 是第一类规模最大、价值最明确的推理执行状态。我们的目标不是建设一个更大的缓存，而是让每一份状态都能被识别、估值、调度和验证消费，让存储和重算成为可比较的获取路径，最终把已经支付过的 Prefill 计算转化为客户可重复兑现的 SLO 合格产能：同样预算完成更多有价值的业务事务，或者同样业务量消耗更少的基础设施成本，而且每一笔收益都可以对账。
...
### 23.1 本项目是否具有独立于 Mooncake、LMCache 的独特价值
...
**尚未。** 目前存在的是一套比“多级缓存”更有辨识度的架构主张。只有在四类决定性实验和 PVT 门槛通过后，它才会从技术品味变成工程能力；只有在客户 BVT、生产价值凭证、硬件认证和生态 ABI 形成后，它才会成为客户愿意购买且竞品难以复制的壁垒。
...
> “在相同模型质量、业务 SLO 和硬件预算下，我们能否让客户完成更多有价值的业务事务；或者在相同业务量下降低全生命周期成本与风险，并用状态决策、实际路径和业务结果三层证据完成对账？”

如果能够回答这一问题，本项目的“推理状态基础设施”理解就形成了完整竞争力闭环：底层软硬件协同决定状态如何获得，状态控制面决定资源价值如何兑现，Customer Value Receipt 证明客户最终得到多少业务回报。即使 Mooncake、LMCache、Dynamo/KVBM/NIXL 持续演进，本项目仍可凭借客户工作负载建模、跨层决策、可审计价值交付和生产数据飞轮形成独立价值；如果最终只能证明多级缓存、RDMA、SSD Offload 或技术 KPI 改善，而无法证明客户业务结果，则仍难以形成长期竞争力。
